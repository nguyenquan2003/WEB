...
<form id="form" class="text-start" method="POST" action="{{route('scores.request_edit.create', ['id' => $rec->id])}}">
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <label class="form-label mt-3">Thang điểm 4</label>
                <div class="input-group input-group-outline">
                    <input disabled type="number" step="0.01" name="tđ" class="form-control" value="{{$rec->tk ?? old('tđ') ?? ''}}">
                </div>
            </div>
            <div class="col-md-6">
                <label class="form-label mt-3">Điểm chữ</label>
                <div class="input-group input-group-outline">
                    <input disabled type="number" step="0.01" name="đc" class="form-control" value="{{$rec->tk ?? old('đc') ?? ''}}">
                </div>
            </div>
            <div class="col-md-6">
                <label class="form-label mt-3">Xếp loại</label>
                <div class="input-group input-group-outline">
                    <input disabled type="number" step="0.01" name="xl" class="form-control" value="{{$rec->tk ?? old('xl') ?? ''}}">
                </div>
            </div>
        </div>

...
class ScoreController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Kiểm tra xem có yêu cầu tìm kiếm không và xử lý tương ứng
        if (!empty($query)) {
            $rows = Score::with(['student.user', 'subject'])
            ...
            $rows = Score::with(['student.user', 'subject'])->get();
        }

        return view('scores.index', compact('rows'));
    }    
